...
final_result_file = out_path + "_x264_cbr_frm%d_"%enc_frames + getDateTime() + ".log"
...
for p in x264_presets:
    ...
    for yuv in test_yuvs:
        ...
        if width <= 1920 and height <= 1080:
            bitrate_array = hd_bitrates
        for bitrate in bitrate_array:
            out_log = "%sx264_%s_%s_cbr%d.log"%(out_path, yuv, p, bitrate)
            bitstream = "%sx264_%s_%s_cbr%d.bin"%(out_path, yuv, p, bitrate)
            cmd    = x264_fullpath + \
                     " --preset %s"%p + \
                     " --tune psnr" + \
                     " --keyint %d"%key_int + \
