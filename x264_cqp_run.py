...
final_result_file = out_path + "_x264_cqp_frm%d_"%enc_frames + getDateTime() + ".log"
...
for p in x264_presets:
    ...
    for yuv in test_yuvs:
        ...
        for qp in fix_quant_qps:
            ...
            fps    = yuv_fps[yuv]
            out_log = "%sx264_%s_%s_cqp%d.log"%(out_path, yuv, p, qp)
            bitstream =  "%sx264_%s_%s_cqp%d.bin"%(out_path, yuv, p, qp)
            cmd    = x264_fullpath + \
                     " --preset %s"%p + \
                     " --tune psnr" + \
                     " --keyint %d"%key_int + \
