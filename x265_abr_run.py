...
final_result_file = out_path + "_x265_abr_frm%d_"%enc_frames + getDateTime() + ".log"
...
for p in x265_presets:
    ...
    for yuv in test_yuvs:
        ...
        if width <= 1920 and height <= 1080:
            bitrate_array = hd_bitrates
        for bitrate in bitrate_array:
            out_log = "%sx265_%s_%s_abr%d.log"%(out_path, yuv, p, bitrate)
            bitstream = "%sx265_%s_%s_abr%d.bin"%(out_path, yuv, p, bitrate)
            cmd    = x265_fullpath + \
                     " --preset %s"%p + \
                     " --tune psnr" + \
                     " --keyint %d"%key_int + \
