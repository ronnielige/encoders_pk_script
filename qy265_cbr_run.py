...
final_result_file = out_path + "_qy265_cbr_result.log"
...
for p in qy265_presets:
    ...
    for yuv in test_yuvs:
        ...
        if width <= 1920 and height <= 1080:
            bitrate_array = hd_bitrates
        for bitrate in bitrate_array:
            out_log = "%sqy265_%s_%s_cbr%d.log"%(out_path, yuv, p, bitrate)
            bitstream =  "%sqy265_%s_%s_cbr%d.bin"%(out_path, yuv, p, bitrate)
            cmd    = qy265_fullpath + \
                     " -preset %s"%p + \
                     " -iper %d"%key_int + \
                     " -bframes %d"%bframes + \
            ...
            if b_cover_result or not is_log_intact(out_log):
                TM.newTaskList((cmd, del_bitstream_cmd,), (out_log, ))
