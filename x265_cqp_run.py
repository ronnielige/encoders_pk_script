...
final_result_file = out_path + "_x265_cqp_frm%d_"%enc_frames + getDateTime() + ".log"
...
for p in x265_presets:
    ...
    for yuv in test_yuvs:
        ...
        for qp in fix_quant_qps:
            ...
            fps    = yuv_fps[yuv]
            out_log = "%sx265_%s_%s_cqp%d.log"%(out_path, yuv, p, qp)
            bitstream =  "%sx265_%s_%s_cqp%d.bin"%(out_path, yuv, p, qp)
            cmd    = x265_fullpath + \
                     " --preset %s"%p + \
                     " --tune psnr" + \
                     " --keyint %d"%key_int + \
